...
\item \textit{No adaptation}. In this experiments we asked a user to follow the robot at the speed that he prefers. The robot will not adapt to the speed of the user, setting its own pace and stopping if the user is too far.
\end{itemize}
...
\item It is important to perform long deployments in similar scenarios with real users. In the three weeks where we worked at the airport, many days were spent adapting the robot to the scenario. Longer deployments would allow for an iterative process of design-implementation-study.
\end{itemize}
