...
\section{Human-Aware Probabilistic Planning}
...
The starting point for this planner was the single agent MDP. We start with the classical model $(S,A,T,C,G,SS)$, where $S$ is the system space, $A$ is the set of actions, $T(s_i,a,s_j)$ is the probability to transition from state $s_i$ to state $s_j$, after taking action $a$, $C(s,a)$ is the cost of taking action $a$ in state $s$, $G \in S$ is the set of goal states, and $SS \in S$ is the set of starting states. We develop this model in the following way:

\subsubsection{Parameters}
We can define parameters in our MDP, and assign them to different values. The list of parameters, and their current values, is called the \textit{parameter_instance} of the model. Parameters can be linked to variables and values. 
...
The MDP receives as input, when consulted to select an action, the state of the world,  which we will call \textit{real_space}. The \textit{real_space} will be converted, using the \textit{parameter_instance} to the \textit{parameter_space}, which will be actually used by the model in its computations. 
...
We define actions as a tuple $(subject,action\_name,object,target)$, where each element of the tuple is called a $part$ of an action.  We represent the action as a string, composed by its $parts$, joined by the the character "_" as deimiter. We define the function $convertPar(a)$ and $convertReal(a)$ to convert action $a$ to the \textit{real_space} or the \textit{parametrized_space}.
...
We define for each MDP class a \textit{name}, which identifies it, and a \textit{parametrized_name}, which includes \textit{parameter_instance}. The \textit{name} is defined using the same tuple as an action. 

For example, an MDP to get object could have as \textit{name} $agent\_get\_object$ and has \textit{parametrized_name}, in a certain moment, $Greg\_get\_book$.

We define a function $assignParameterFromActionName(a)$ which creates the \textit{parameter_instance} of the MDP based on an action name. This function can be very useful when using \textit{macro actions}, by assigning parameters to the MDP from the \textit{macro action} and then consulting the sub-MDP.

For example, if the \textit{macro action} $Greg\_get\_book$ is linked to the $agent\_get\_object$ MDP, this function would assign to the $agent$ parameter the value $Greg$ and to the $object$ parameter the value $book$.
...
$SS=\bigcap_{k=1}^n (SS_k)$, where $SS_k$ is the set of starting states of a single agent MDP. The starting states of the MAMDP is set to the intersection of the single agents starting states.

$G=\bigcup_{k=1}^n (G_k) $, where $G_k$ is the set of goal states of a single agent MDP. The goal states of the MAMDP is the intersection of the single agent goal states. The idea of this choice is that an MDP terminates when one of the agent has achieved its goal. At this point, if there is a higher level 
...
We require two things to create an MAMDP: a $name$, which will be used to save its informations, and a list of $n$ MDPs $MDP_k$, one for each agent. 

The action space of the joint model is calculated as the cross product of the action spaces of the single model. When there are several macro actions in the agent models, this could result in the creation and solving of several sub-models, which can significantly slow down the computation of a solution for the model. We can reduce this process by making the following considerations:
\begin{itemize}
\item We can save compute MAMDPs and reuse them in different branches of the hierarchy, if needed. When creating a new MAMDP, we will look to see if there is already an existing MAMDP with the same $name$ and, if so, we will use it.
\item We can use the concept of parameters to reduce the number of compute sub-MAMDPs. For each macro action $a$, we first define the list of needed sub-MDPs, as previously explained. We create the parametrized name of the sub-MAMDP by concatenating the parametrized name of these sub-MDPs. We first set the $name$ of the sub-MAMDP as the concatenation of the $name$ of the sub-MDPs, and then edit each of its $part$s in the following way:
$name_i=\begin{cases}
	name_i & \quad \text{if } !parametersInCommon(name_i)
	parametrized\_name_i & \quad \text{if } !parametersInCommon(name_i,i)
\end{cases}$ where $parametersInCommon(name_i)$ is a function that returns true if $name_i$ is a parameter in sub-MDP $MDP_i$, and there is a sub-MDP $MDP_j$ where $name_i$ is a parameter or variable. \\

The idea behind this choice is the following. If the sub-MDPs do not have parameter in common, we can use a generic MAMDP to represent this instance. For example, if we create an MAMDP for the macro action $agent1_clean_table-agent2_clean_shelf$, we can use a generic MAMDP $agent_agent_glue-agent_get_book$ since the sub-MDP models do not have parameters in common. If instead we create an MAMDP for the macro action $agent1_clean_table-agent2_clean_table$ we create a specialized MAMDP, since there are parameters in common ($table$) and if we treat them as different objects there could be incongruences in the MAMDP planning.
\end{itemize}
