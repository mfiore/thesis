...
% First, we introduce the main aspects of human-robot cooperation, and present a general overview of our system, which is divided in different layers. Perception data is analyzed by a Situation Assessment layer, which is able to produce symbolic information and to maintain humans' belief. Additionally, this component is able to reason on humans activities to infer their most likely intention. A Goal Management layer handles the robot's goals, which can be received by humans or generated by the system, by reasoning on the state of the world and on the most likely humans' intentions. After producing a goal, the system creates a shared plan, considering all the agents that collaborate in the scenario. When managing a shared plan, the robot will execute its part of the plan, while monitoring humans' actions, to ensure that they are following the strategy. Our system is also able to explain shared plans to human, adapting this process to their knowledge about the tasks to perform. Finally, the Task Execution layer executes the robot's actions in a safe and robust way. We also present a special framework based on Mixed Observability Markov Decision Processes, that we called Collaborative Planners, used to execute joint actions between the robot and humans.
...
\textbf{TPNU} & \textbf{T}emporal \textbf{P}lan \textbf{N}etwork with \textbf{U}ncertainty \\
\textbf{I-POMDP} & \textbf{I}nteractive \textbf{P}artially \textbf{O}bservable \textbf{M}arkov \textbf{D}ecision \textbf{P}rocess \\ 
...
\input{parts/case_study/case_study.tex}
\input{parts/conclusions/conclusions_part.tex}
...
\newpage
\appendix
\input{parts/conclusions/methods.tex}
